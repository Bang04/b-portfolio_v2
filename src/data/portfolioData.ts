 ...
 *
 * ───────────────────────────────────────────────────────────────────────────
 * 2026-09-15 최신 커밋 로그·문서 재대조 후 갱신.
 *  - 커밋 수 "300여 건" → "387건"(재확인된 정확한 수치)으로 두 곳 수정
 *    (dd-verify-alternative.approach, turningPoints t-08-mid.description)
 *  - featuredProject.team: "프론트엔드 2·백엔드 2(총 4명)"처럼 검증 안 되는
 *    특정 인원수 대신, git 저자 기준으로 실제 확인되는 사실(슬로프 웹
 *    도메인은 단독 개발, 모바일 웹뷰는 별도 담당)로 문구 완화
 *  - featuredProject.role.scope: 실서버 API 105건의 연동 상태를 실측해
 *    문서로 관리한 습관(81.9% 연동)과 재현 증거 첨부 방식의 백엔드 협업을
 *    한 문장으로 추가 — 협업 근거가 얇았던 부분 보강
 *  - deepDives 5개 → 7개: dd-auth-bypass(공통 인증 버그를 고치지 않고
 *    도메인 안에서 우회), dd-stale-closure(지도 클릭 stale closure 버그)
 *    추가. "15개→5개로 줄인다"는 원칙에 대한 의도적 예외로, 기존 카드가
 *    다루지 않던 판단 유형(권한/책임 경계 준수, React 상태관리 근본기)을
 *    보여주기 때문에 채택
 *  - featuredProject.team: "웹 도메인 단독 개발"이 안전점검 워크플로(limits
 *    에 이미 "다른 프론트엔드 개발자 담당"으로 고지된 영역)까지 포함하는
 *    것처럼 읽혀 자기 모순 → "실태조사·지도관제 도메인 단독 개발(안전점검·
 *    모바일 웹뷰는 별도 담당)"으로 범위를 실제 담당 영역만큼 좁힘
 *  - atAGlance 섹션 신설: 커밋 수·담당 파일 수·API 연동율·협업 인원을
 *    헤더 바로 아래 스탯 카드로 모음. role.scope에 흩어져 있던 API 연동율
 *    (81.9%)은 중복이라 본문에서 빼고 이 카드로만 남김 — 대신 role.scope엔
 *    실제로 만든 화면(등록·수정 통합 폼, HWP·PDF 보고서 엔진, 다층 권한
 *    가드)을 한 문장으로 추가해 "무엇을 만들었나"가 비어 있던 부분을 보강
 * ------------------------------------------------------------------------- */
...
const profile: PortfolioData['profile'] = {
  ...
  cta: {
    ...
    secondary: { label: '포트폴리오 요약 (PDF)', href: '/resume.pdf', download: true },
  },
 ...
 *
 * 이후 두 차례 더 추가해 7개가 됐다. 둘 다 "개수를 줄인다" 원칙에 대한
 * 의도적 예외인데, 앞의 4~5개가 다루지 않은 판단 유형이라 중복이 아니기
 * 때문이다 — dd-verify-alternative(검증 습관), dd-auth-bypass(권한 밖
 * 공통 코드를 만났을 때의 책임 경계 판단), dd-stale-closure(React
 * 상태관리의 근본적인 함정). 다음에 추가할 소재가 이 4가지 판단 유형
 * 중 하나와 겹친다면, 새 카드 대신 기존 카드를 보강하는 쪽을 먼저
 * 검토한다.
 * ------------------------------------------------------------------------- */
const deepDives: PortfolioData['featuredProject']['deepDives'] = [
  ...
  {
    id: 'dd-auth-bypass',
    title: '검토 승인 API만 401 — 공통 코드는 고치지 않고 우회한 이유',
    tag: '인증',
    problem:
      '검토 승인/반려 버튼만 401이 나는데, 같은 로그인 상태로 다른 API는 전부 정상 동작했습니다. 원인은 다섯 플랫폼이 공유하는 공통 axios 인터셉터가 토큰을 Bearer 접두어 없이 raw로 전송하고 있었던 것이었고, 마침 승인 API 서버만 접두어를 엄격히 요구했습니다.',
    approach:
      '원인은 공통 코드에 있었지만, 그 코드는 다른 네 플랫폼이 동일하게 의존하고 있어 프로젝트 규칙상 수정 대상이 아니었습니다. 그래서 공통 인스턴스를 고치는 대신, 승인 계열 API 전용 axios 인스턴스를 도메인 폴더 안에 새로 만들어 그쪽으로만 우회했습니다.',
    result:
      '승인/반려 흐름이 정상화됐고 다른 네 플랫폼에는 변경이 전혀 가지 않았습니다. 근본 원인(공통 인터셉터 버그)은 공통 모듈 담당팀에 보고 대상으로 남겨뒀습니다.',
    metric: {
      label: '승인 API 정상화',
      before: '401 (다른 API는 정상)',
      after: '전용 인스턴스로 우회 후 정상',
      delta: '공통 코드 변경 0건',
    },
    learned:
      '"고치면 더 빠르다"와 "고쳐도 되는가"는 다른 질문입니다. 원인이 공통 코드에 있어도 그 코드를 고칠 권한과 책임이 없다면, 문제를 먼저 내 영역 안으로 가져와 푸는 방법을 찾아야 했습니다.',
    keywords: ['공통 셸 격리', '전용 axios 인스턴스', '우회 설계', '책임 경계'],
  },
  {
    id: 'dd-stale-closure',
    title: '지도 종점을 클릭하면 0.5초 뒤 되돌아가던 버그',
    tag: '상태',
    problem:
      '지도에서 급경사지 종점을 클릭하면 좌표가 잠깐 반영됐다가 0.5초 뒤 원래 위치로 되돌아갔습니다. 지도 SDK의 클릭 이벤트 핸들러가 리액트 훅 의존성 배열 바깥에 있어, 의존성 배열을 채우는 일반적인 접근으로는 원인이 보이지 않았습니다.',
    approach:
      '핸들러가 실행되는 시점의 최신 좌표가 아니라, 핸들러가 정의될 당시의 오래된 state를 그대로 참조하고 있다는 걸 확인했습니다(stale closure). 그 자리만 고치는 대신 같은 유형의 버그가 재발하지 않도록 규칙을 세웠습니다 — 리렌더가 필요 없는 값은 ref로 관리하고, 상태 갱신은 항상 함수형 업데이터로 최신값 위에서만 하고, 여러 필드가 얽힌 갱신은 공용 유틸(mergeSpec) 하나로 모아 처리했습니다.',
    result:
      '지도 클릭 좌표 버그가 해소됐고, 이후 폼 상태 전반에서 같은 유형(stale closure)의 버그가 재발하지 않았습니다.',
    metric: {
      label: '지도 좌표 반영',
      before: '0.5초 후 원위치로 복귀',
      after: '클릭한 좌표 유지',
      delta: '재발 방지 규칙 3종 적용',
    },
    learned:
      '의존성 배열을 채우는 것만으로는 못 고치는 stale closure도 있습니다. 이벤트 핸들러가 리액트 훅 밖에 있을 땐, 최신값을 다시 구독하는 방법보다 애초에 오래될 수 없는 형태(ref, 함수형 업데이트)로 설계를 바꾸는 쪽이 근본적인 해법이었습니다.',
    keywords: ['stale closure', 'ref 규약', '함수형 업데이터', '지도 SDK 이벤트'],
  },
  {
  ...
  approach:
    '문서를 그대로 믿는 대신 실제 slope·photo 레코드를 대상으로 대안 API를 직접 호출해 전/후 상태를 비교했고, 그 결과 대안 역시 이동 방식이라는 걸 확인한 뒤 기각했습니다. 대신 사진을 원본 그대로 재업로드해 새 회차에 별도 행을 생성하도록 재설계했습니다. 이후 이 프로젝트의 커밋 387건을 정리해 이력서·회고 자료를 쓸 때도 같은 원칙을 적용했습니다 — AI로 커밋을 태그·근거 기준 1차 분류해 후보를 좁혔지만, 이 사진 버그처럼 핵심 판단이 걸린 항목은 AI 요약을 그대로 쓰지 않고 직접 재현해 확인했고, 수치 근거가 없는 항목은 AI 응답에 등장했더라도 "확인 필요"로 분리해 회고에서 제외했습니다.',
  result:
...
const turningPoints: PortfolioData['featuredProject']['turningPoints'] = [
  ...
  },
  {
    ...
    title: '검증 없이는 믿지 않기',
    description:
      '사진 이관 버그를 서버가 준 대안 API까지 직접 검증해 기각하고, 같은 원칙으로 387개 커밋을 정리한 회고 문서를 남겼습니다.',
...
const featuredProject: PortfolioData['featuredProject'] = {
  ...
  period: '2026.06 – 진행 중',
  domain: '재난안전 · 급경사지',
  team: '프론트엔드 — 급경사지 실태조사·지도관제 도메인 단독 개발(안전점검·모바일 웹뷰는 별도 담당) · 백엔드팀과 협업',
  client: '지자체 행정 실무자용', // 발주처명은 공개 확인 전까지 익명 유지

  atAGlance: {
    asOf: '2026-09-15',
    note: '실제 git 로그·docs·코드를 근거로 뽑은 프로젝트 규모입니다.',
    stats: [
      {
        id: 'glance-commits',
        label: '내 커밋',
        value: '387건',
        description: '전체 1,633건 중 · 2026-06-09 ~ 09-15',
      },
      {
        id: 'glance-files',
        label: '담당 화면 파일',
        value: '200+개',
        description: 'src/features/ahplus_slope 하위 .tsx',
      },
      {
        id: 'glance-api',
        label: 'API 연동율',
        value: '81.9%',
        description: '105건 중 86건 연동(부분연동 포함 89.5%)',
      },
      {
        id: 'glance-team',
        label: '함께 일한 인원',
        value: '2명 + 백엔드팀',
        description: '모바일 웹뷰 1명 · 문서/설정 1명(문서상 이니셜만 확인)',
      },
    ],
  },
  ...
  role: {
    scope:
      '초기 기획·화면설계·데이터 명세서 작성 담당 후, 급경사지 실태조사 워크플로 개발부터 재해위험도 등급 산정 로직, 관리자 설정·데이터 로그 화면까지 급경사지 도메인 프론트엔드 전반을 담당. 5단계 위저드였던 등록·수정 폼을 단일 폼으로 리팩터링하고, HWP·PDF 보고서 다운로드 엔진과 등급별 다층 권한 가드를 직접 구현. API 연동 상태는 직접 실측해 문서로 관리했고, 계약이 어긋난 항목은 재현 증거를 첨부한 요청서로 백엔드팀과 협의',
  },
